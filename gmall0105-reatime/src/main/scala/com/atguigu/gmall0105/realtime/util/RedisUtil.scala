...
/**
  * Created by jxy on 2020/12/18 0018 12:21
  */
object RedisUtil {
  ...
  def getJedisClient: Jedis = {
    if(jedisPool==null){
      ...
      val database = config.getProperty("redis.database")
      ...
      jedisPool=new JedisPool(jedisPoolConfig,host,port.toInt,1000,password,database.toInt)
    }
